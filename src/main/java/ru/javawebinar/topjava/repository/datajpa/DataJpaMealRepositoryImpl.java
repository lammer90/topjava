...
public class DataJpaMealRepositoryImpl implements MealRepository {
    ...
    public Meal getWithUser(int id, int userId) {
        Meal meal = crudMealRepository.getWihtUser(id, userId).stream().findFirst().orElse(null);
    ...
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return crudMealRepository.getAll(userId).stream().filter(m -> Util.isBetween(m.getDateTime(), startDate, endDate)).collect(Collectors.toList());
    }
}
